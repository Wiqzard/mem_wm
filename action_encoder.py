...
class ActionEncoder(nn.Module):
    ...
    the final output has shape (B, (T//l) * 7, l * hidden_dim).
    """
    ...
    def __init__(self, hidden_dim=32, num_frames=20, group_size=None):
        ...
        # -- 2) Single embeddings for space, shift, mouse_1, mouse_2
        # Each is a learnable vector of size [hidden_dim].
        ...
        # -- 4) Temporal embeddings: one learnable embedding per frame index
        # If you have a maximum of num_frames, we can embed up to that index
        self.frame_embedding_table = nn.Embedding(num_frames, hidden_dim)
        self.mask_token = nn.Parameter(torch.randn(1, hidden_dim * group_size))
